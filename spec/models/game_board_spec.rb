...
RSpec.describe GameBoard, :type => :model do
  ...
  let(:pawn1) { GamePiece.create(char_x: 'C', x: 3, y: 2, type: 'Pawn', game_id: 0, game_board_id: 1, created_at: Time.now, updated_at: Time.now) }

  it "looks for all game pieces it has" do
    expect(pawn1.game_board.to eq('Pawn'))
  end
